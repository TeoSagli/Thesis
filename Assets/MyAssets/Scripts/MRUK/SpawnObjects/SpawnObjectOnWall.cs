...
/// </summary>
public class SpawnObjectOnWall : Spawn
{
        ...
        for (int i = 0; i < SpawnAmount; ++i)
        {
            ...
            for (int j = 0; j < MaxIterations; ++j)
            {
                ...
                if (SpawnObject.gameObject.scene.path == null)
                {
                    ...
                    spawnPosition.y = playerHead.position.y;
                    GameObject obj = Instantiate(SpawnObject, spawnPosition, spawnRotation, transform);
                    var s = obj.transform.localScale;
                    var ps = transform.localScale;
                    obj.transform.localScale = new(s.x * ps.x, s.y * ps.y, s.z * ps.z);
                    float checkBound = 0.25f;
                    int choice = GetPrefabOrientation(obj.transform.position, checkBound, room);
                    OrientatePrefabToDirection(obj, choice, checkBound);
                }
                else
                {
    ...
    /// <param name="room">The room to spawn objects in.</param>
    private int GetPrefabOrientation(Vector3 obj, float offset, MRUKRoom room)
    {
        float px = obj.x;
        float py = obj.y;
        float pz = obj.z;
    ...
    private void OrientatePrefabToDirection(GameObject prefab, int choice, float offset)
    {
        float px = prefab.transform.position.x;
        float py = prefab.transform.position.y;
        float pz = prefab.transform.position.z;
        switch (choice)
        {
            case 0:
